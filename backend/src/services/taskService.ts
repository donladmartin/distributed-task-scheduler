...
// WebSocket setup
const wss: Server = new WebSocketServer({ port: 8080, host: "0.0.0.0" });

const initializeTasks = async () => {
  const tasks = await Task.find();
  tasks.forEach((task) =>
    (!task.executed && task.time) || !task.time
      ? scheduleTask(task.toObject() as ITask)
      : ""
  );
};

const connectRabbitMQ = (
  callback: (err: any, connection: amqp.Connection) => void,
  retries = 5
) => {
...
export const scheduleTask = (task: ITask) => {
...
connectRabbitMQ((err, connection) => {
  ...
  connection.createChannel((err, ch) => {
    ...
    ch.consume("task_queue", async (msg) => {
      if (msg !== null) {
        const task: ITask = JSON.parse(msg.content.toString());
        console.log(`Executing task: ${task._id}`);

        const existingTask = await Task.findById(task._id);
        if (!existingTask) {
          console.log(
            `Task ${task._id} not found in the database. Removing from queue.`
          );
          ch.ack(msg);
          return;
        }
